# AGB und Stornoregel — Entwurf

Stand: 2026-08-27. Status: Entwurf zur anwaltlichen Prüfung.

> **Kein Rechtsrat.** Ich bin kein Anwalt und darf keine Rechtsberatung
> leisten. Der Text unten ist ein Entwurf, der Marianas Vorgaben in
> Klauseln übersetzt. Vor der ersten Buchung von einem Anwalt prüfen
> lassen: Eine unwirksame Klausel fällt ersatzlos weg, und dann gilt das
> Gesetz — meist zu Ungunsten des Verwenders.

---

## Der Ablauf, wie Mariana ihn will

```
Buchung  →  Sometra schreibt Rechnung  →  Zahlung binnen 7 Tagen  →  Platz bestätigt
                          ↑
                  48 Stunden Storno ab hier
```

Keine Zahlung, kein Workshop. Kein Lastschrifteinzug, keine Rechnung nach
dem Workshop. Beides hat in der Vergangenheit nicht funktioniert.

---

## Ein Widerspruch, der aufgelöst werden musste

In Marianas Vorgabe stecken zwei Fristen, die sich überlagern:

- **48 Stunden** Stornofrist ab Buchung
- **7 Tage** Zahlungsfrist ab Rechnung

Das erzeugt ein Loch: Wer nach 51 Stunden merkt, dass er doch nicht kann,
darf nicht mehr stornieren — und zahlt einfach nicht. Nach dem Wortlaut
schuldet er trotzdem den vollen Preis, aber der Platz ist blockiert, und
Mariana müsste mahnen. Genau das soll der Ablauf ja verhindern.

**Aufgelöst so:** Die 48 Stunden laufen ab **Rechnungsdatum**, nicht ab
Buchung. Danach ist die Buchung verbindlich. Bleibt die Zahlung aus, darf
Sometra den Platz weitergeben — der Anspruch auf das Honorar bleibt
bestehen, solange der Platz nicht neu besetzt ist. Damit ist der Platz nach
sieben Tagen wieder frei, und der Anspruch ist trotzdem nicht verschenkt.

**Und eine Klausel, die Ihnen Ärger erspart:** Ein Ersatzteilnehmer aus
demselben Unternehmen ist kostenfrei zulässig. Das kostet Sie nichts, weil
der Platz ohnehin bezahlt ist — und es nimmt fast jeder Absage die Schärfe.
Ohne diese Klausel diskutieren Sie im November über Kulanz.

---

## Entwurf: Allgemeine Geschäftsbedingungen

**Sometra · Mariana Theux**
Workshop „So bauen Sie sich Ihre Vertriebsagenten"
Stand: [Datum eintragen]

### § 1 Geltungsbereich und Vertragspartner

(1) Diese Bedingungen gelten für alle Buchungen des Präsenz-Workshops,
veranstaltet von [**vollständige Firmierung der zypriotischen Gesellschaft,
Registernummer, eingetragener Sitz, USt-IdNr. eintragen**], nachfolgend
„Sometra".

(2) **Das Angebot richtet sich ausschließlich an Unternehmer im Sinne des
§ 14 BGB, an juristische Personen des öffentlichen Rechts und an
öffentlich-rechtliche Sondervermögen. Eine Buchung durch Verbraucher ist
ausgeschlossen.** Der Teilnehmer bestätigt bei der Buchung, dass er in
Ausübung seiner gewerblichen oder selbständigen beruflichen Tätigkeit
handelt.

(3) Abweichende Bedingungen des Teilnehmers gelten nur, wenn Sometra ihnen
schriftlich zustimmt.

### § 2 Zustandekommen des Vertrags

(1) Die Darstellung des Workshops auf der Website ist kein bindendes
Angebot.

(2) Mit der Buchung gibt der Teilnehmer ein verbindliches Angebot ab.

(3) Der Vertrag kommt zustande, wenn Sometra die Buchung durch Zusendung
der Rechnung annimmt.

### § 3 Preis und Zahlung

(1) Der Teilnahmepreis beträgt **2.356 € netto** je Person, zuzüglich der
gesetzlichen Umsatzsteuer.

(2) Der Preis ist **innerhalb von sieben Kalendertagen ab Rechnungsdatum**
und in jedem Fall vor Veranstaltungsbeginn per Überweisung zu zahlen.

(3) **Der Platz ist erst mit vollständigem Zahlungseingang verbindlich
bestätigt.** Bis dahin ist er lediglich reserviert.

(4) Geht die Zahlung nicht fristgerecht ein, ist Sometra berechtigt, den
Platz anderweitig zu vergeben und vom Vertrag zurückzutreten. Der Anspruch
auf die vereinbarte Vergütung bleibt unberührt, soweit der Platz nicht
anderweitig besetzt werden kann.

(5) Ohne Zahlungseingang besteht kein Anspruch auf Teilnahme.

### § 4 Stornierung durch den Teilnehmer

(1) Der Teilnehmer kann die Buchung **innerhalb von 48 Stunden ab
Rechnungsdatum** kostenfrei in Textform stornieren.

(2) **Nach Ablauf dieser Frist ist eine Stornierung ausgeschlossen.** Der
volle Teilnahmepreis bleibt zur Zahlung fällig, auch wenn der Teilnehmer
nicht erscheint.

(3) Der Teilnehmer kann jederzeit und kostenfrei **einen Ersatzteilnehmer
aus seinem Unternehmen benennen.** Die Benennung erfolgt in Textform,
spätestens 24 Stunden vor Veranstaltungsbeginn.

(4) Ein gesetzliches Widerrufsrecht besteht nicht, da der Vertrag
ausschließlich mit Unternehmern geschlossen wird.

(5) Sometra kann dem Teilnehmer bei einer Absage nach Ablauf der Frist aus
Absatz 1 **nach eigenem Ermessen** einen Gutschein über den gezahlten Betrag
anbieten. Der Gutschein ist zwölf Monate ab Ausstellung gültig, auf jeden
von Sometra angebotenen Workshop anrechenbar und innerhalb des Unternehmens
des Teilnehmers übertragbar. **Ein Anspruch auf einen Gutschein besteht
nicht.**

### § 5 Absage oder Verlegung durch Sometra

(1) Sometra kann den Workshop bis 14 Tage vor dem Termin absagen oder auf
einen Ersatztermin verlegen, insbesondere wenn

a) **weniger als sechs Anmeldungen** für den Termin vorliegen, oder
b) die Durchführung aus Gründen unmöglich wird, die Sometra nicht zu
   vertreten hat, etwa Krankheit, höhere Gewalt oder Ausfall der
   Räumlichkeiten.

(2) **Sometra ist nicht verpflichtet, in der Absage den Grund zu nennen.**

(3) Der gezahlte Teilnahmepreis wird in diesem Fall vollständig erstattet.
Wahlweise kann der Teilnehmer auf einen Ersatztermin umbuchen.

(4) Weitergehende Ansprüche, insbesondere auf Ersatz von Reise- oder
Übernachtungskosten, bestehen nicht. Reise und Unterkunft organisiert und
trägt der Teilnehmer selbst.

### § 6 Ergebnisgarantie

(1) Sometra sagt zu, dass am Ende des Workshoptages **mindestens ein vom
Teilnehmer selbst erstellter Agent bei ihm lauffähig ist.**

(2) Ist das nicht der Fall, wird der volle Teilnahmepreis erstattet.

(3) Voraussetzung ist, dass der Teilnehmer ganztägig anwesend war, an den
Bauabschnitten mitgearbeitet hat und die technischen Voraussetzungen nach
§ 7 erfüllt.

(4) Die Garantie ist **noch am Veranstaltungstag vor Ende der
Veranstaltung** gegenüber Sometra geltend zu machen, damit der Sachverhalt
gemeinsam festgestellt werden kann. Die Erstattung erfolgt innerhalb von
14 Tagen.

### § 7 Mitwirkung und technische Voraussetzungen

(1) Der Teilnehmer bringt einen eigenen, uneingeschränkt nutzbaren Laptop
mit sowie Zugang zu einem Google-Konto.

(2) Sometra teilt spätestens sieben Tage vor dem Termin mit, welche
weiteren Zugänge benötigt werden. Der Teilnehmer richtet diese vor dem
Termin ein und bestätigt dies auf Nachfrage.

(3) Kann ein Teilnehmer wegen fehlender oder gesperrter Zugänge nicht
mitarbeiten, berührt das den Zahlungsanspruch nicht, und die Garantie nach
§ 6 entfällt.

### § 8 Unterlagen und Nutzungsrechte

(1) Alle Unterlagen, Vorlagen und Anleitungen sind urheberrechtlich
geschützt.

(2) Der Teilnehmer darf sie **innerhalb seines eigenen Unternehmens
uneingeschränkt nutzen, vervielfältigen und an Mitarbeiter weitergeben.**

(3) Eine Weitergabe an Dritte außerhalb des Unternehmens, eine
Veröffentlichung oder ein Weiterverkauf sind nicht gestattet.

(4) Aufzeichnungen des Workshops durch Teilnehmer sind nur mit vorheriger
Zustimmung von Sometra zulässig.

### § 9 Haftung

(1) Sometra haftet unbeschränkt bei Vorsatz und grober Fahrlässigkeit sowie
bei Verletzung von Leben, Körper oder Gesundheit.

(2) Bei einfacher Fahrlässigkeit haftet Sometra nur bei Verletzung einer
wesentlichen Vertragspflicht und beschränkt auf den vertragstypischen,
vorhersehbaren Schaden.

(3) Sometra schuldet die Vermittlung von Wissen und die Begleitung beim
Aufbau. **Ein bestimmter wirtschaftlicher Erfolg wird nicht geschuldet**,
mit Ausnahme der Zusage nach § 6.

(4) Für den Betrieb der vom Teilnehmer erstellten Agenten nach dem Workshop
ist der Teilnehmer allein verantwortlich, insbesondere für die Einhaltung
der Nutzungsbedingungen der eingesetzten Dienste und der
datenschutzrechtlichen Vorgaben in seinem Unternehmen.

### § 10 Datenschutz und Aufnahmen

(1) Sometra verarbeitet die Buchungsdaten zur Durchführung des Vertrags.
Einzelheiten in der Datenschutzerklärung.

(2) Fotos oder Videoaufnahmen während der Veranstaltung werden nur mit
vorheriger Einwilligung der Abgebildeten angefertigt und verwendet.

### § 11 Schlussbestimmungen

(1) Es gilt [**Rechtswahl eintragen — siehe Hinweis unten**].

(2) Ausschließlicher Gerichtsstand für alle Streitigkeiten aus diesem
Vertrag ist **Zypern, [Stadt eintragen]**, soweit der Teilnehmer Unternehmer,
juristische Person des öffentlichen Rechts oder öffentlich-rechtliches
Sondervermögen ist. Die Gerichtsstandsvereinbarung wird nach Art. 25 der
Verordnung (EU) Nr. 1215/2012 getroffen.

(3) Änderungen und Ergänzungen bedürfen der Textform.

(4) Sollte eine Bestimmung unwirksam sein, bleibt der Vertrag im Übrigen
wirksam.

---

## Was Mariana noch entscheiden muss

| Punkt | Warum |
|---|---|
| ~~Mindestteilnehmerzahl~~ | **erledigt: sechs.** Bei fünf oder weniger wird abgesagt |
| ~~Gerichtsstand~~ | **erledigt: Zypern.** Stadt noch eintragen. Folgen siehe oben |
| **Rechtswahl: deutsches oder zypriotisches Recht?** | Blockiert die Prüfung. Der Entwurf ist auf deutschem Recht gebaut |
| **Vollständige Firmierung, Registernummer, Sitz, USt-IdNr.** | Gehört in § 1 und ins Impressum |
| **Umsatzsteuer: Eintrittsberechtigung oder Grundregel?** | Entscheidet über deutsche Registrierung und über den Preistext auf der Seite |
| **Sind 48 Stunden ab Rechnung gemeint oder ab Buchung?** | Der Entwurf sagt: ab Rechnung. Das ist die einzige Fassung, die mit der 7-Tage-Zahlungsfrist zusammenpasst |
| **Ersatzteilnehmer-Klausel behalten?** | Empfehlung: ja. Sie kostet nichts und nimmt fast jeder Absage die Schärfe |

---

## Sometra ist zypriotisch — was daran hängt

Stand 27.08. Diese Information kam nach dem ersten Entwurf und ändert drei
Dinge.

### 1. Gerichtsstand Zypern ist zulässig — und kostet Sie zweimal

Zwischen Unternehmern ist eine Gerichtsstandsvereinbarung nach Art. 25
Brüssel-Ia-Verordnung wirksam, auch in AGB. Rechtlich geht das. **Umgesetzt
ist es in § 11.**

Zwei Kosten, die Sie kennen sollten:

- **Beim Verkaufen.** Ein Geschäftsführer, der 2.803 € an eine ihm unbekannte
  ausländische Gesellschaft überweisen soll und in den AGB „Gerichtsstand
  Zypern" liest, wird vorsichtiger, nicht mutiger. Das ist der Absatz, den
  eine Rechtsabteilung als Erstes anstreicht
- **Beim Eintreiben.** Wenn ein Teilnehmer nicht zahlt, müssen **Sie** in
  Zypern klagen und das Urteil danach in Deutschland vollstrecken. Für eine
  Forderung von 2.356 € lohnt das nie. Mit deutschem Gerichtsstand stünde
  Ihnen das Europäische Mahnverfahren offen, und das ist billig und schnell

Ein Mittelweg, der beides löst und den ausländische Anbieter im
Deutschlandgeschäft regelmäßig wählen: **deutsches Recht und deutscher
Gerichtsstand für das Deutschlandgeschäft**, unabhängig davon, wo die
Gesellschaft sitzt. Ihre Entscheidung — im Entwurf steht Zypern, wie
gewünscht.

### 2. Die Rechtswahl ist jetzt offen

Der gesamte Entwurf ist auf deutschem Recht gebaut: § 9 folgt der deutschen
Haftungssystematik, § 4 der deutschen AGB-Kontrolle nach § 307 BGB.

- **Deutsches Recht gewählt** → der Entwurf trägt, ein deutscher Anwalt
  prüft ihn
- **Zypriotisches Recht gewählt** → der Entwurf ist falsch verankert und
  gehört zu einem zypriotischen Anwalt. Ich kann ihn dafür nicht anpassen

Solange das offen ist, ist § 11 Absatz 1 eine Lücke, keine Klausel.

### 3. Die Umsatzsteuer ist nicht mehr der einfache Fall

Steht ausführlich in `angebot/zahlungsweg.md`. Kurz: Ob deutsche
Umsatzsteuer mit deutscher Registrierung oder Reverse-Charge, hängt daran,
ob der Workshop als Eintrittsberechtigung zu einer Veranstaltung oder als
sonstige Leistung nach der B2B-Grundregel gilt. **Vor dem ersten verkauften
Platz mit dem Steuerberater klären.** Davon hängt auch ab, was auf der
Landingpage beim Preis steht.

---

## Rückerstattung oder Gutschein

Marianas Frage: keine Stornierung anbieten, stattdessen einen Gutschein für
das nächste Seminar. Machen viele so.

**Empfehlung: beides, aber getrennt.**

### Die 48 Stunden mit Rückerstattung behalten

Ein Fremder soll 2.803 € an eine ausländische Gesellschaft überweisen, für
einen Workshop in neun Wochen, den es noch nie gab. Die 48-Stunden-Regel ist
neben der Ergebnisgarantie das stärkste Vertrauenselement, das die Seite
hat.

Und sie kostet fast nichts: Wer nach 48 Stunden storniert, hat sich meist
schon beim Buchen geirrt. Diese Rückerstattungen sind selten. Sie gegen ein
paar gesparte Fälle einzutauschen, ist ein schlechter Tausch — Sie zahlen
mit Buchungen, die gar nicht erst zustande kommen.

### Den Gutschein als Ihr Recht formulieren, nicht als seines

So steht es jetzt in § 4 Absatz 5: Sometra **kann** einen Gutschein
anbieten, ein Anspruch besteht nicht.

Der Unterschied ist entscheidend. Als Anspruch des Teilnehmers wird der
Gutschein zur regulären Ausstiegstür, und die feste Buchung ist wieder weg.
Als Ihr Ermessen ist er das Werkzeug, mit dem Sie den einen Fall lösen, bei
dem jemand wirklich nicht kann — ohne dass alle anderen davon lernen.

### Drei Bedingungen, damit ein Gutschein kein Problem wird

- **Zwölf Monate Gültigkeit.** Ein Gutschein für „das nächste Seminar" ist
  wertlos, solange für 2027 kein Termin steht
- **Übertragbar im Unternehmen.** Sonst verfällt er, wenn die Person wechselt
- **Nicht bei der Ergebnisgarantie.** Wenn § 6 greift, fließt Geld zurück,
  kein Gutschein. Sonst ist die Garantie auf der Landingpage keine
  Geld-zurück-Garantie mehr, und genau das steht dort

---

## Absagen, ohne die Teilnehmerzahl preiszugeben

Marianas Anliegen: Wenn zu wenige buchen, sollen die Angemeldeten nicht
erfahren, dass es an der Teilnehmerzahl lag.

**Das geht — ohne einen falschen Grund zu nennen.** § 5 Absatz 2 stellt
klar, dass Sometra den Grund nicht angeben muss. Die Absage nennt dann
schlicht keinen. Damit erfährt niemand etwas über die Buchungslage, und es
steht keine Behauptung im Raum, die später jemand nachprüfen kann.

**Warum der Umweg über eine vorgeschobene Krankheit die schlechtere Lösung
ist**, ganz unabhängig davon, wie man es moralisch sieht:

- **Ohne die Klausel in Absatz 1a gibt es kein Absagerecht wegen zu geringer
  Teilnehmerzahl.** Wer dann absagt und Krankheit vorschiebt, sagt ohne
  vertragliche Grundlage ab. Der Haftungsausschluss für Reise- und
  Hotelkosten in Absatz 4 greift nur für Absagen nach Absatz 1. Ein
  Teilnehmer mit gebuchtem Hotelzimmer könnte die Kosten verlangen
- **Der Markt ist klein.** Zwölf Plätze, drei Städte, dieselbe Branche. Ein
  Absagegrund, den zwei Teilnehmer nebeneinanderlegen können, hält nicht
- **Es ist unnötig.** „Der Termin findet nicht statt, Sie bekommen den
  vollen Betrag zurück, hier sind die Ersatztermine" verrät nichts und
  behauptet nichts

### Vorlage für die Absage-Mail

> Betreff: Workshop am [Datum] in [Stadt] — Termin entfällt
>
> Guten Tag [Name],
>
> der Workshop am [Datum] in [Stadt] findet nicht statt. Den vollen Betrag
> von [Betrag] € überweise ich Ihnen bis [Datum] zurück, ohne dass Sie
> etwas tun müssen.
>
> Wenn Sie möchten, buche ich Sie auf einen der anderen Termine um:
> [Stadt, Datum] oder [Stadt, Datum]. Ihr Platz ist dort reserviert, ein
> Wort von Ihnen genügt.
>
> Es tut mir leid für Ihre Planung.
>
> Mariana Theux

Kein Grund, keine Ausrede, keine Angriffsfläche. Und ein Umbuchungsangebot,
das den Teilnehmer möglicherweise rettet, statt ihn zu verlieren.

---

## Was auf der Landingpage noch fehlt

Beides ist Pflicht und beides fehlt heute:

- **Impressum** — Anbieterkennzeichnung
- **Datenschutzerklärung** — Informationspflichten bei der Datenerhebung
  über das Buchungsformular

Dazu ein **Link auf die AGB im Buchungsablauf** mit Bestätigung durch den
Teilnehmer. Ohne Einbeziehung gelten die AGB nicht — dann greift das
Gesetz, und die 48-Stunden-Regel ist wertlos.

---

## Digistore24 — die Frage, ob es damit anders wäre

Nein. Es wäre teurer, nicht billiger.

**Am Preismodell nachgerechnet** (Websuche, nicht am Dienst geprüft):
7,9 % plus 1 € auf die ersten 400 €, darüber 4,9 %, bei Vorkasse 2,9 %.
**Keine Grund- oder Jahresgebühr.**

Bruttopreis je Platz: 2.356 € × 1,19 = **2.803,64 €**

| | je Platz | 36 Plätze |
|---|---|---|
| Standard | **150 €** | **5.414 €** |
| Vorkasse | **102 €** | **3.683 €** |

Die erinnerten 500 € im Jahr waren also vermutlich etwas anderes — nach
aktueller Auskunft gibt es bei Digistore keine Jahresgebühr. Der Kostenblock
ist die Transaktionsgebühr, und die wächst mit dem Ticketpreis. Bei 2.356 €
je Platz ist das der teuerste denkbare Fall.

**Zwei Gründe, die schwerer wiegen als das Geld:**

1. **Digistore verkauft als Wiederverkäufer.** Die Rechnung kommt von
   Digistore, nicht von Sometra. Marianas Ablauf lautet aber: „ich schreibe
   Rechnung". Das schließt sich aus.
2. **Digistore ist für Sofortzahlung gebaut** — Karte, PayPal, Lastschrift.
   Der gewünschte Ablauf ist Rechnung plus sieben Tage Überweisung. Das
   geht dort zwar als Vorkasse, aber dann zahlt Mariana Gebühren für einen
   Ablauf, den sie ohnehin von Hand macht.

**Für diese Runde also: keine Plattform.** Buchungsformular, Rechnung,
Überweisung, Tabelle. Kosten 0 €.

---

## Quellen

- [Features & Preise — Digistore24](https://www.digistore24.com/de/home/features)
- [Digistore24 Testbericht und Gebühren — Oliver Pfeil](https://www.oliverpfeil.de/online-tools/digistore24)
- [Digistore24 Pricing — OMR Reviews](https://omr.com/en/reviews/product/digistore24/pricing)
